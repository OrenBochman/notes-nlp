...
Welcome to week two of this specialization. We will learn about Naive Bayes. Concretely, we will be using Naive Bayes for sentiment analysis on tweets. Given a tweet, we will decide if it has a positive sentiment or a negative one. Specifically we will: 
...
We  may want to browse the documentation of unfamiliar libraries and functions.
...
If we are running this notebook in your local computer,
...
- **Remove noise**: We will first want to remove noise from your data -- that is, remove words that don't tell we much about the content. These include all common words like 'I, you, are, is, etc...' that would not give us enough information on the sentiment.
- We'll also remove stock market tickers, retweet symbols, hyperlinks, and hashtags because they can not tell we a lot of information on the sentiment.
...
We have given we the function `process_tweet()` that does this for you.
...
To help train your naive bayes model, we will need to build a dictionary where the keys are a (word, label) tuple and the values are the corresponding frequency.  Note that the labels we'll use here are 1 for positive and 0 for negative.
...
For example: given a list of tweets `["i am rather excited", "we are rather happy"]` and the label 1, the function will return a dictionary that contains the following key-value pairs:
...
<ul>
    ...
    <li>We may find it useful to use the `zip` function to match each element in `tweets` with each element in `ys`.</li>
    ...
    <li>Assume that the `result` dictionary that is input will contain clean key-value pairs (we can assume that the values will be integers that can be incremented).  It is good practice to check the datatype before incrementing the value, but it's not required here.</li>
</ul>
</p>
</details>
...
- Given your `count_tweets()` function, we can compute a dictionary called `freqs` that contains all the frequencies.
...
##### Calculate $V$
- We can then compute the number of unique words that appear in the `freqs` dictionary to get your $V$ (we can use the `set` function).
...
In this part we can predict the sentiment of your own tweet.
...
Congratulations on completing this assignment. See we next week!
