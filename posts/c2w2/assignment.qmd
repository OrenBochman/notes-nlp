...
description: "Develop skills in part-of-speech (POS) tagging, the process of assigning a part-of-speech tag (Noun, Verb, Adjective...) to each word in an input text."
categories: 
  - NLP 
  - Coursera 
  - Lab
...
- We are doing **well** for yourself. [adjective]
...
# Part 1: Parts-of-speech tagging  {#1}


## Part 1.1 - Training {#1.1}
...
![](img/pos.png){ style="width:400px;height:250px;" group="slides"}
...
In order for we to compute equation 1, we will create a `transition_counts` dictionary where 
...
In order for we to compute equation 2, we will create an `emission_counts` dictionary where 
...
- We can get a more complete description at [Penn Treebank II tag set](https://www.clips.uantwerpen.be/pages/mbsp-tags). 
...
### Part 1.2 - Testing {#1.2}

Now we will test the accuracy of your parts-of-speech tagger using your `emission_counts` dictionary. 
- Given your preprocessed test corpus `prep`, we will assign a parts-of-speech tag to every word in that corpus. 
...
# Part 2: Hidden Markov Models for POS {#2}
...
## Part 2.1 Generating Matrices {#2.1}
...
This will allow we to quickly construct the 
...
# Part 3: Viterbi Algorithm and Dynamic Programming {#3}

In this part of the assignment we will implement the Viterbi algorithm which makes use of dynamic programming. Specifically, we will use your two matrices, `A` and `B` to compute the Viterbi algorithm. We have decomposed this process into three main steps for you. 
...
* **Feed backward**: This allows we to find the best path with the highest probabilities. 


## Part 3.1:  Initialization {#3.1}
...
## Part 3.2 Viterbi Forward (prediction) {#3.2}
...
# Part 4: Predicting on a data set {#4}
...
- To split y into the word and its tag we can use `y.split()`. 
...
Congratulations we were able to classify the parts-of-speech with 95% accuracy. 
...
- Bidirectional POS tagging would tell we more about the POS instead of just knowing the previous word. 
...
If like me we find this challenging why not:

- TODO: implement the HMM trifecta
    - [ ] Forward–backward - given a sequence of outputs, infer the most likely sequence of states
    - [ ] Viterbi - Compute the most likely trajectory 
    - [ ] Baum-Welch - Find the unknown parameters of a hidden Markov model (HMM) needs needs the FB alg 
- TODO: implement Viterbi, etc+ related algorithms for spark
- TODO: code the Viterbi, etc in Manim to visualize the algorithm!...
:::