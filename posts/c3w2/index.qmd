...
Note that you end up training $W_{hh}$, $W_{hx}$, $W_{yh}$, $b_h$, and $b_y$. Here is a visualization of the model.


![](img/slide06.png){.column-margin}
...
![](img/slide07.png){.column-margin}

you are basically summing over the all the classes and then multiplying  $y_j$ times $log(\hat{y}_j)$. 
If you were to compute the loss over several time steps, use the following formula: 
...
![](img/slide09.png){.column-margin}

Note that at layer $l$, you are using the input from the bottom  $a^{[;-1]}$ and the hidden state $h^{<l>}$ That allows you to get your new h, and then to get your new a, you will train another weight matrix $W_{a}$, which you will multiply by the corresponding h add the bias and then run it through an activation layer.
