...
We will now implement your first machine translation system and then you
will see how locality sensitive hashing works. Let's get started by importing
the required functions!

If we are running this notebook in your local computer, don't forget to
...
If we want to run this on your local computer and use the full dataset,
we can download the
...
<summary>
    ...
    <p>
        <ul>
            ...
            <li>We can get words which are embedded into the language by using <a href="https://www.w3schools.com/python/ref_dictionary_keys.asp"> keys</a> method.</li>
            <li>Keep vectors in `X` and `Y` sorted in list. We can use <a href="https://docs.scipy.org/doc/numpy-1.13.0/reference/generated/numpy.ma.vstack.html"> np.vstack()</a> to merge them into the numpy matrix. </li>
...
# We do not have to input any code in this cell, but it is relevant to grading, so please do not change anything
...
<ul>
   ...
   <li> Try to use matrix operations instead of the numpy norm function.  If we choose to use norm function, take care of extra arguments and that it's returning loss squared, and not the loss itself.</li>
...
<p>
<ul>
    <li> We cannot rely on training loss getting low -- what we really want is the validation loss to go down, or validation accuracy to go up. And indeed - in some cases people train until validation accuracy reaches a threshold, or -- commonly known as "early stopping" -- until the validation accuracy starts to go down, which is a sign of over-fitting.
    </li>
    <li>
    Why not always do "early stopping"? Well, mostly because well-regularized models on larger data-sets never stop improving. Especially in NLP, we can often continue training for months and the model will continue getting slightly and slightly better. This is also the reason why it's hard to just stop at a threshold -- unless there's an external customer setting the threshold, why stop, where do we put the threshold?
    </li>
    <li>Stopping after a certain number of steps has the advantage that we know how long your training will take - so we can keep some sanity and not train for months. We can then try to get the best performance within this time budget. Another advantage is that we can fix your learning rate schedule -- e.g., lower the learning rate at 10% before finish, and then again more at 1% before finishing. Such learning rate schedules help a lot, but are harder to do if we don't know how long you're training.
    </li>
</ul>
</p>
...
* Now we can use [numpy argsort]( https://docs.scipy.org/doc/numpy/reference/generated/numpy.argsort.html#numpy.argsort) to sort the indices for the rows of `candidates`.
...
of the English embedding we have just transformed.
...
We managed to translate words from one language to another language
...
In this part of the assignment, we will implement a more efficient version
...
<p>
<ul>
    <li> We can handle missing words easier by using the `get()` method of the python dictionary instead of the bracket notation (i.e. "[ ]"). See more about it <a href="https://stackoverflow.com/a/11041421/12816433" >here</a> </li>
    ...
    <li>  We can use your `process_tweet()` function which allows we to process the tweet. The function just takes in a tweet and returns a list of words.</li>
</ul>
</p>
...
# this gives we a similar tweet as your input.
...
```
@zoeeylim sad sad sad kid :( it's ok I help we watch the match HAHAHAHAHA
```
...
* If we calculate the dot product with each plane in the same order for every vector, you've encoded each vector's unique hash ID as a binary number, like [0, 1, 1, ... 0].
...
* First multiply your vector `v`, with a corresponding plane. This will give we a vector of dimension $(1,\text{N_planes})$.
* We will then convert every element in that vector to 0 or 1.
* We create a hash vector by doing the following: if the element is negative, it becomes a 0, otherwise we change it to a 1.
...
Given that we have a unique number for each vector (or tweet), We now want to create a hash table. We need a hash table, so that given a hash_id, we can quickly look up the corresponding vectors. This allows we to reduce your search by a significant amount of time.
...
We have given we the `make_hash_table` function, which maps the tweet vectors to a bucket and stores the vector there. It returns the `hash_table` and the `id_table`. The `id_table` allows we know which vector in a certain bucket corresponds to what tweet.
...
We can now hash your vectors and store them in a hash table that
would allow we to quickly look up and search for similar vectors.
Run the cell below to create the hashes. By doing so, we end up having
several tables which have all the vectors. Given a vector, we then
identify the buckets in all the tables.  We can then iterate over the
buckets and consider much fewer vectors. The more buckets we use, the
...
# 4 Conclusion
Congratulations - Now we can look up vectors that are similar to the
